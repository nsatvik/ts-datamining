...
\begin{figure}[h!]
	\centering
		\includegraphics[scale=0.8]{screenshots/sax/breakpoint_table.png}
		\caption{A lookup table that contains the breakpoints that divide a Gaussian
distribution in an arbitrary number (from 3 to 10) of equiprobable regions}
\end{figure}}

\item{\textbf{Discretization }- Once the breakpoints have been obtained we can discretize a time series in the following manner. We
...
smallest breakpoint are mapped to the symbol “b”, etc, as shown in Figure 4.8

\begin{figure}[h!]
	\centering
		\includegraphics[scale=0.8]{screenshots/sax/discretization.png}
		\caption{A time series is discretized by first obtaining a PAA approximation and
...
\item{\textbf{Distance Measure} - Having introduced the new representation of time series, we can now define a distance measure on it. The distance between
...
\subsection{Temporal Pattern Mining Algorithm}
Genetic Algorithms are an adaptive search technique based on the principles
and mechanisms of natural selection. The Temporal Pattern Mining Framework is a novel method for discovering
patterns characteristic of time series events. A genetic algorithm
...
“This framework adapts and innovates
...
\begin{itemize}
\item{An \textbf{event} is an indication of an important occurrence. An event is defined
in terms of the goal of the framework, the type of event we want to predict.}

\item{A \textbf{temporal pattern} is a hidden structure in a time series that is characteristic
and predictive of events. To characterize events, we
first need uncover temporal patterns, these are patterns hidden in the time
series. A pattern p is represented
